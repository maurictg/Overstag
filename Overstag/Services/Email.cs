...
namespace Overstag.Services
{
    ...
    public class PassWarningMail : Email
    {
        public PassWarningMail(Account user) : base("Wachtwoord gewijzigd", "","","noreply@stoverstag.nl", null,null)
        {
            ...
            this.to.Clear();
            this.to.Add(user.Email);
            this.message = $"<h1>Iemand heeft uw wachtwoord veranderd</h1><h4>Beste {user.User.FirstName}, <br>Op {DateTime.Now.ToString("dd-MM-yyyy")} om {DateTime.Now.ToString("HH:mm:ss")} heeft iemand uw wachtwoord veranderd.<br><b>Was u dit zelf? Beschouw deze mail dan als niet verzonden</b>.<br>Als je deze activiteit niet herkent, neem dan contact met ons op.</h4>";
        }
    }

    public class PassresetMail : PassWarningMail
    {
        public PassresetMail(Account user, string token) : base(user)
        {
            this.message = "<h1>Overstag wachtwoord reset</h1>" +
                                    "Beste " + user.User.FirstName + ",<br>We sturen je deze mail omdat je je wachtwoord vergeten bent.<br>" +
                                    "Klik op <a href='http://stoverstag.nl/Register/Passreset/" + Uri.EscapeDataString(token) + "'>deze link</a>  om je wachtwoord te resetten of plak hem in je adresbalk." +
                                    "Als de link het niet doet, kopieer en plak deze dan: http://stoverstag.nl/Register/Passreset/" + Uri.EscapeDataString(token) + "<br>" +
